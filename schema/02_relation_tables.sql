...
CREATE TABLE "SERVER_SOLU" (
    ...
    CONSTRAINT "FK_SS_SERVER" FOREIGN KEY ("SERVER_ID") REFERENCES "MST_SERVER"("SERVER_ID") ON DELETE CASCADE,
    CONSTRAINT "FK_SS_SOLU" FOREIGN KEY ("SOLU_ID") REFERENCES "MST_SOLU"("SOLU_ID") ON DELETE CASCADE
);
...
CREATE TABLE "SERVER_STAFF" (
    ...
    CONSTRAINT "FK_SRVS_STAFF_SERVER" FOREIGN KEY ("SERVER_ID") REFERENCES "MST_SERVER"("SERVER_ID") ON DELETE CASCADE,
    CONSTRAINT "FK_SRVS_STAFF_STAFF" FOREIGN KEY ("STAFF_ID") REFERENCES "MST_STAFF"("STAFF_ID") ON DELETE CASCADE
);
...
CREATE TABLE "SOLU_STAFF" (
    ...
    CONSTRAINT "FK_SOLS_STAFF_SOL" FOREIGN KEY ("SOLU_ID") REFERENCES "MST_SOLU"("SOLU_ID") ON DELETE CASCADE,
    CONSTRAINT "FK_SOLS_STAFF_STAFF" FOREIGN KEY ("STAFF_ID") REFERENCES "MST_STAFF"("STAFF_ID") ON DELETE CASCADE
);

COMMENT ON TABLE "SOLU_STAFF" IS '솔루션 담당자 (1차/2차 각 1명)';


-- 공동작업 그룹별 담당자 (그룹 N : 담당자 N)
CREATE TABLE "COWORK_GRP_STAFF" (
    "COWORK_GRP_STAFF_ID" NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- PK
    "COWORK_GRP_ID"       NUMBER(10) NOT NULL,                                 -- 공동작업그룹 ID
    "STAFF_ID"            NUMBER(10) NOT NULL,                                -- 담당자 ID
    "DISPLAY_ORDER"       NUMBER(5) DEFAULT 0,                                -- 표시 순서
    "CREATED_AT"          TIMESTAMP DEFAULT SYSTIMESTAMP,                      -- 등록일시
    CONSTRAINT "UK_COWORK_GRP_STAFF" UNIQUE ("COWORK_GRP_ID", "STAFF_ID"),
    CONSTRAINT "FK_CGS_GRP" FOREIGN KEY ("COWORK_GRP_ID") REFERENCES "COWORK_GRP_MASTER"("COWORK_GRP_ID") ON DELETE CASCADE,
    CONSTRAINT "FK_CGS_STAFF" FOREIGN KEY ("STAFF_ID") REFERENCES "MST_STAFF"("STAFF_ID") ON DELETE CASCADE
);

COMMENT ON TABLE "COWORK_GRP_STAFF" IS '공동작업 그룹별 담당자';
COMMENT ON COLUMN "COWORK_GRP_STAFF"."COWORK_GRP_STAFF_ID" IS 'PK';
COMMENT ON COLUMN "COWORK_GRP_STAFF"."COWORK_GRP_ID" IS '공동작업그룹 ID';
COMMENT ON COLUMN "COWORK_GRP_STAFF"."STAFF_ID" IS '담당자 ID';
COMMENT ON COLUMN "COWORK_GRP_STAFF"."DISPLAY_ORDER" IS '표시 순서';
