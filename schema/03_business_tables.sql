...
CREATE TABLE "PATCH_MANAGEMENT" (
    ...
    CONSTRAINT "FK_PM_SOLU" FOREIGN KEY ("SOLU_ID") REFERENCES "SOLU_MASTER"("SOLU_ID") ON DELETE SET NULL,
...
COMMENT ON COLUMN "PATCH_MANAGEMENT"."SOLU_ID" IS '대상 솔루션 (NULL이면 서버만 패치)';
...
CREATE TABLE "REGULAR_INSPECTION" (
    ...
    CONSTRAINT "FK_RI_SOLU" FOREIGN KEY ("SOLU_ID") REFERENCES "SOLU_MASTER"("SOLU_ID") ON DELETE SET NULL
);

COMMENT ON TABLE "REGULAR_INSPECTION" IS '정기점검업무 정보 (벤더 방문 점검)';
COMMENT ON COLUMN "REGULAR_INSPECTION"."INSPECTION_ID" IS '점검 ID';
COMMENT ON COLUMN "REGULAR_INSPECTION"."VENDOR_NAME" IS '방문 벤더명';
COMMENT ON COLUMN "REGULAR_INSPECTION"."SERVER_ID" IS '점검 대상 서버';
COMMENT ON COLUMN "REGULAR_INSPECTION"."SOLU_ID" IS '점검 대상 솔루션';
...
CREATE INDEX "IDX_PATCH_MANAGEMENT_SOLU" ON "PATCH_MANAGEMENT"("SOLU_ID");
...
CREATE INDEX "IDX_REGULAR_INSPECTION_VENDOR" ON "REGULAR_INSPECTION"("VENDOR_NAME");
...
CREATE INDEX "IDX_REGULAR_INSPECTION_SOLU" ON "REGULAR_INSPECTION"("SOLU_ID");
